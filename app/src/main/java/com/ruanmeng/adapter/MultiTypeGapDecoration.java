...
public class MultiTypeGapDecoration extends RecyclerView.ItemDecoration {

    private static final String TAG = MultiTypeGapDecoration.class.getSimpleName();
    private int gapSize;
    private boolean offsetTopEnabled;

    public MultiTypeGapDecoration() {
        gapSize = DensityUtil.dp2px(10);
    }

    public MultiTypeGapDecoration(int gapSize) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (isHeadView(parent, adapterPostion) || isFootView(parent, adapterPostion)) {
            //头部视图 或 底部视图
            outRect.set(0, 0, 0, 0);
        } else {
            //数据视图
    ...
    public boolean isHeadView(RecyclerView recyclerView, int position) {
        ...
        return adapter instanceof HeaderAndFooterRecyclerViewAdapter && ((HeaderAndFooterRecyclerViewAdapter) adapter).isHeader(position);
    }

    public boolean isFootView(RecyclerView recyclerView, int position) {
        RecyclerView.Adapter adapter = recyclerView.getAdapter();
        return adapter instanceof HeaderAndFooterRecyclerViewAdapter && ((HeaderAndFooterRecyclerViewAdapter) adapter).isFooter(position);
    }

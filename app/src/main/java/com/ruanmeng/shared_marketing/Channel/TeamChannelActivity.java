...
public class TeamChannelActivity extends BaseActivity {
    ...
    private String[] items = new String[]{ "未推荐", "已推荐", "去电", "到访", "认购", "签约", "结佣", "失效" };
    ...
    private void setIndicator(Indicator indicator) {
        ...
        indicator.setOnItemSelectListener(new Indicator.OnItemSelectedListener() {
            ...
            public void onItemSelected(View selectItemView, int select, int preSelect) {
                ...
                pageNum = 1;
                getData(pageNum, mStatus = select + 1);
            }
        });
    ...
    public void getData(final int pindex, int status) {
        mRequest = NoHttp.createStringRequest(HttpIP.recList, Const.POST);
        mRequest.add("partner_id", getIntent().getStringExtra("id"));
        mRequest.add("pindex", pindex);
        mRequest.add("cus_status", status);
        ...
        getRequest(new CustomHttpListener<CustomerData>(baseContext, true, CustomerData.class) {
                    ...
                    public void doWork(CustomerData data, String code) {
                        ...
                        list.addAll(data.getData().getUser());
                        adapter.notifyDataSetChanged();

                        if (data.getData().getList().size() > 0) {
                            list_count.clear();
                            list_count.addAll(data.getData().getList());
                    ...
                    public void onFinally(JSONObject obj, String code, boolean isSucceed) {
                        ...
                        try {
                            if (obj.getJSONObject("data").getJSONArray("user").length() > 0) {

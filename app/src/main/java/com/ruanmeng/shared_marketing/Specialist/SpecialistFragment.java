...
public class SpecialistFragment extends Fragment {
    ...
    ImageView iv_msg;
    @BindView(R.id.iv_fragment_company_search)
    ImageView iv_search;
    ...
    private int[] mRes = {
            R.mipmap.icon_group1,
            R.mipmap.icon_group2,
            R.mipmap.icon_group3,
            R.mipmap.icon_group4,
            R.mipmap.icon_group5,
            R.mipmap.icon_group6 };
    ...
    private void init() {
        ...
        adapter = new CommonAdapter<CommonData.CommonInfo>(getActivity(), R.layout.item_specialist_list, list) {
            @Override
            protected void convert(ViewHolder holder, final CommonData.CommonInfo info, int position) {
                holder.setText(R.id.tv_item_specialist_group, info.getDepartment_name());
                holder.setText(R.id.tv_item_specialist_num, info.getCus_count() + "人");
                // holder.setImageResource(R.id.iv_item_specialist_img, mRes[position % mRes.length]);

                RoundedImageView iv_img = holder.getView(R.id.iv_item_specialist_img);
                Glide.with(mContext)
                        .load(info.getIcon())
                ...
                holder.setVisible(R.id.v_item_specialist_divider_1, holder.getLayoutPosition() + 1 != mDatas.size());
                holder.setVisible(R.id.v_item_specialist_divider_2, holder.getLayoutPosition() + 1 == mDatas.size());

                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent intent = new Intent(getActivity(), TeamSpecialistActivity.class);
                        intent.putExtra("id", info.getId());
                        intent.putExtra("name", info.getDepartment_name());
    ...
    public void getData() {
        mRequest = NoHttp.createStringRequest(HttpIP.departmentList, Const.POST);
